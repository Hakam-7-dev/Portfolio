...
const DotMatrix: React.FC<DotMatrixProps> = ({
  ...
  return <Shader source={fragmentShader(center, shader)} uniforms={uniforms} />;
};

const fragmentShader = (center: ("x" | "y")[], shader: string) => `
...
const ShaderMaterialMesh = ({
  ...
  useFrame(({ clock }) => {
    ...
    const materialInstance = meshRef.current.material;

    if (materialInstance instanceof THREE.ShaderMaterial) {
      materialInstance.uniforms.u_time.value = t;
      materialInstance.uniforms.u_resolution.value.set(
        size.width * 2,
        size.height * 2
      );
    }
